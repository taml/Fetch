<script setup>
    ...
    const { dogBreed, favouriteToggled } = storeToRefs(dogStore)
    ...
    // Hide the selected paw icon under certain conditions
    const itemNotSelected = () => {
        if(props.subBreed || favouriteToggled.value || (props.breed !== dogBreed.value)) return true
    }
</script>

<template>
    <a class="breed-link" href="javascript:void(0)" :aria-selected="!props.subBreed && !favouriteToggled && breed === dogBreed">
        {{ breed }}
